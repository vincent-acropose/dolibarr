...
$searchprecis=GETPOST("searchprecis");
...
print '<input type="hidden" name="actioncode" value="AC_TEL">';
print '<input type="hidden" name="action" value="show_day">';

print '<select name="status" onchange="this.form.submit()">';
print '<option value="" '.(($status == '')?'selected="selected"' : '') .'>tous</option>';
print '<option value="done" '.(($status == 'done')?'selected="selected"' : '') .'>téléphone repondu ou appelé</option>';
print '<option value="todo" '.(($status == 'todo')?'selected="selected"' : '') .'>appel manqué</option>';
print '</select>';
...
if(!empty($searchprecis)) {
$sql.= " AND (";
$sql.= " a.label LIKE '%_".$searchprecis."_%' ";
$sql.= ')';
}
...
// Sort on date
$sql.= ' GROUP BY a.datep2, a.label, a.fk_soc, a.fk_contact';
$sql.= ' ORDER BY datep2 DESC, percent ASC';
//print $sql;
...
if (empty($action) || $action == 'show_month')      // View by month
{
    ...
    $newparam.='&viewcal=1';
    $newparam.='&searchprecis='.$searchprecis;
