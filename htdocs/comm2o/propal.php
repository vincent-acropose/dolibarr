...
// 	require_once DOL_DOCUMENT_ROOT . '/core/class/html.formprojet.class.php';
	...
	dol_include_once('/comm2o/class/html.formprojet2o.class.php');
}
...
// Create proposal
	...
	if (! $error) {
		...
		if (GETPOST('createmode') == 'copy' && GETPOST('copie_propal')) {
			...
			/**
			  Extract project and use in propale 
			*/
// 			option_customfields_numeroclientmutuelle

			
			$_POST['options_ref_mutuel_nomeclature'] = GETPOST('option_customfields_claturemut_idclassement_numnomenclatures');
			$_POST['options_mutuel_customer'] = GETPOST('option_customfields_choixmutuelleprojet_idprof6_nom');
...
if ($action == 'create') {
	...
	if (! empty($conf->projet->enabled) && $socid > 0) {
		...
		$formproject = new FormProjets2o($db);
// print_r($formproject); 
		$projectid = 0;
		if ($origin == 'project')
		...
		$idprojet = $formproject->selected_projects($soc->id, $projectid);
...
// 		}
	  ...
	  $extralabelsP=$extrafieldsP->fetch_name_optionals_label($objectP->table_element);
	  $objectP->fetch($projectid);
	  $objectP->fetch_optionals($objectP->id,$extralabelsP);
//     print_r($extralabelsP);
			  // Other options
  //         $parameters=array();
//   print_r($objectP); 
//   exit; 

// print('Module context: '.$objectP->context);
	  $myaction = 'myinfo';
	  $parameters = 'edit';
	  $reshook=$hookmanager->executeHooks('formObjectOptions',$parameters,$objectP,$myaction); // Note that $action and $object may have been modified by hook
// 	  
...
// // 	  exit; 
// 	  
	  if (empty($reshook) && ! empty($extrafieldsP->attribute_label))
	  {
		  print $objectP->showOptionals($extrafieldsP, 'edit');
	  }
	...
	print '<tr class="noborder"><td></td><td colspan="2" class="noborder" ><table width="100%"  class="border nopadding" >';
...
// 	  EXIT; 
	print '<tr><td height="10">' . $langs->trans('RembousementMutuel') . '</td>';
	print '<td align="right" class="nowrap">' . price( (float)$nomenclature->intervoa, '', $langs, 0, - 1, - 1, $conf->currency) . '</td>';
	print '</tr>';
	
	print '<tr><td height="10">' . $langs->trans('QuotePartRestanteTTC') . '</td>';
	print '<td align="right" class="nowrap"><b>' . price( ($object->total_ttc - $nomenclature->intervoa) , '', $langs, 0, - 1, - 1, $conf->currency) . '</b></td>';
	print '</tr>';
	
	print '<tr><td height="10">' . $langs->trans('QuotePartRestanteTTCPaimPartiel') . '</td>';
	print '<td align="right" class="nowrap"><b>' . price( round((($object->total_ttc - $nomenclature->intervoa)/ $object->array_options['options_nbr_caution']),2), '', $langs, 0, - 1, - 1, $conf->currency) . '</b></td>';
	print '</tr>';
	
	print '</table></td></tr>';
	// Statut
	print '<tr><td height="10">' . $langs->trans('Status') . '</td><td align="left" colspan="2">' . $object->getLibStatut(4) . '</td></tr>';
// {cf_projdevis_ref_cf_claturemut_idclassement_numnomenclatures_intervoa}
	print '</table><br>';
