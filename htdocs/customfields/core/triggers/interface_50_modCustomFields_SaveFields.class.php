...
// trigger_error('-----------------'.$action); 
				 ...
				 $dirname = substr(dirname(__FILE__), 0, - 27); 
// var_dump();
// exit;
				 /// OSCIM FIX 
				if( $_SERVER['SCRIPT_NAME'] == "/htdocs/calling/calling.php"  /*&& isset($_GET['callee']) && isset($_GET['caller']) &&  !isset($_GET['doli'])*/ )
					return 1; 
...
include_once($dirname . '/customfields/class/customfields.class.php');
//             include_once(dirname(__FILE__).'/../../class/customfields.class.php');
              //! FIX Oscim 
//             if (file_exists('/customfields/fields/customfields_fields_extend.lib.php')) dol_include_once('/customfields/fields/customfields_fields_extend.lib.php');
if (file_exists($dirname . '/customfields/fields/customfields_fields_extend.lib.php')) 
include_once($dirname . '/customfields/fields/customfields_fields_extend.lib.php');
...
// trigger_error( print_r($customfields, true ) ); 
	    ...
	    //! FIX Oscim 
//             dol_include_once('customfields/conf/conf_customfields.lib.php');
...
//             dol_include_once('customfields/conf/conf_customfields_func.lib.php');
            ...
            include_once($dirname . '/customfields/conf/conf_customfields_func.lib.php');

	    // Generic trigger based on the trigger array
	    if (preg_match('/^('.implode('|',array_keys((array)@$triggersarray)).')$/i', $action, $matches) ) { // if the current action is on a supported trigger action
		dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);
	    ...
	    $patternsarray = array();
	    foreach ((array)@$modulesarray as $module) { // we create a pattern for regexp with contexts and modules names mixed
