...
class Equipement extends CommonObject
{
	...
	function create()
	{
		...
		{
			if ($conf->global->EQUIPEMENT_SEPARATORLIST)
				$tblSerial=explode($conf->global->EQUIPEMENT_SEPARATORLIST, $this->SerialFourn);
			else
				$tblSerial=explode(";", $this->SerialFourn);
	...
	function set_fact_client($user, $fk_fact_client)
	{
        ...
        /*NON sense to have invoice older != from destinated company */
		...
		if ($user->rights->equipement->creer)
		{
			...
			if ($this->db->query($sql))
			{
				$this->fk_fact_fourn = $fk_fact_fourn;
                                // retrieve fourn, sure to have it because of constraint
                                $factTmp= new FactureFournisseur($this->db);
                                $factTmp->fetch($this->fk_fact_fourn);
                                $fk_soc_fourn = $factTmp->socid;

                                $sql = "UPDATE ".MAIN_DB_PREFIX."equipement ";
                                $sql.= " SET fk_soc_fourn = ".($fk_soc_fourn!=-1? $fk_soc_fourn:"null");
                                $sql.= " WHERE rowid = ".$this->id;
                                $sql.= " AND entity = ".$conf->entity;
                                //$sql.= " AND fk_statut = 0";
                                ...
                                if ($this->db->query($sql))
                                {
                                        $this->fk_soc_fourn = $fk_soc_fourn;
                                        return 1;
                                }
                                else
                                {
                                        $this->error=$this->db->error();
                                        dol_syslog("Equipement::set_fact_client Erreur SQL on fk_soc_fourn update");
                                        return -2;
                                }
			}
			else
