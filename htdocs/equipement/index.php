...
{
	$statProducts.= "<tr >";
	$statProducts.= '<td><a href="list.php?fk_etatequipement='.$objp->fk_etatequipement.'">'.$langs->trans($objp->etatequiplibelle).'</a></td><td align="right">'.$objp->total.'</td>';
	$statProducts.= "</tr>";
...
$sql = "SELECT COUNT(ee.rowid) as total, ee.fk_equipementevt_type, eet.libelle as statueventlibelle";
...
if ($result)
{
	...
	{
		$statProducts.= "<tr >";
		$statProducts.= '<td><a href="listEvt.php?fk_equipementevt_type='.$objp->fk_equipementevt_type.'">'.$langs->trans($objp->statueventlibelle).'</a></td><td align="right">'.$objp->total.'</td>';
		$statProducts.= "</tr>";
		$total=$total+$objp->total;
	}
	print $statProducts;
...
if ($result)
{
	...
	if ($num > 0)
	{
		...
		while ($i < $num)
		{
			...
			print "<td>".$langs->trans($objp->etatequimement)."</td>";
...
if ($result)
{
	...
	if ($num > 0)
	{
		...
		while ($i < $num)
		{
			...
			print "</td>\n";
			print '<td>'.$objp->refproduit. " - ".dol_trunc($objp->label,32).'</td>';
			print "<td>".$langs->trans($objp->statueventlibelle)."</td>";
			print "<td>";
