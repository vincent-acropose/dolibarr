...
else if ($action == 'editfournisseur' )
{
	$object->fetch($id);
        $new_fourn = GETPOST('fk_soc_fourn','int');
        // retrieve fournisseur on facture
        $ff = new FactureFournisseur($db);
        $return = $ff->fetch($object->fk_fact_fourn);
        if( $object->fk_fact_fourn != 0 && $return && $ff->fournisseur != $new_fourn ){
            // mode confirmation
            if($confirm == 'yes'){
                $result=$object->setValueFrom('fk_soc_fourn', $new_fourn);
                $result=$object->setValueFrom('fk_facture_fourn', 0);
                header("Location: ".dol_buildpath("/equipement/card.php?id=$id",2));
            } else {
                $action = 'ask_replacefacture' ;
            }
            $result = 0;
        } elseif($new_fourn != 0) {
            // mode normal
...
else if ($id > 0 || ! empty($ref))
{
	...
	if ($action == 'ask_deleteline')
	{
        ...
        if($action == 'ask_replacefacture')
	{

                $ret=$form->form_confirm($_SERVER["PHP_SELF"].'?id='.$object->id.'&fk_soc_fourn='.$new_fourn,$langs->trans('Confirmation: cette action va supprimer la facture fournisseur'),$langs->trans('&Ecirc;tes vous sur de vouloir changer de fournisseur et de supprimer l\'&eacute;quipement de la facture d\'achat ?'), 'editfournisseur','',0,1);
		if ($ret == 'html') print '<br>';
	}
	...
	// fournisseur
        $LstSupplier=$prod->list_suppliers();
	print '<tr><td class="fieldrequired"><table class="nobordernopadding" width="100%"><tr><td>'.$langs->trans("Fournisseur").' <span style="font-weight:normal">('.count($LstSupplier).')</span></td><td align="right">';
        if ($object->fk_soc_fourn == 0 ) {
            if(count($LstSupplier)==0){
                print '<a href="'.dol_buildpath('/product/fournisseurs.php?id='.$object->fk_product, 2).'">'.img_picto("","edit_add").'</a>';
            } elseif($action != 'editfournisseur') {
                print '<a href="'.$_SERVER["PHP_SELF"].'?action=editfournisseur&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a>';
            }
        } elseif($action != 'editfournisseur') {
            print '<a href="'.$_SERVER["PHP_SELF"].'?action=editfournisseur&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a>';
        }
        print '</td></table> </td><td align="left">';

	if ($object->fk_soc_fourn > 0 && $action != 'editfournisseur')
	{
                $soc=new Societe($db);
                $soc->fetch($object->fk_soc_fourn);
                print $soc->getNomUrl(1);
	} elseif ($action == 'editfournisseur') {
                print '<form name="editfournisseur" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post">';
                print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
                print '<input type="hidden" name="action" value="editfournisseur">';
                ...
                if (count($LstSupplier) ==1)
                {
                        ...
                        //on vire le dernier 'or' et on rajoute la parenthèse de fin
                        $filterList= "( ".substr($filterList, 0, -2).")";
                        print $form->select_company($object->fk_soc_fourn,'fk_soc_fourn', $filterList,1);
                }


                print '<input type="submit" class="button" value="'.$langs->trans('Save').'"> '.$langs->trans('OU');
                print ' <a href="'.dol_buildpath('/product/fournisseurs.php?id='.$object->fk_product, 2).'" class="button" style="font-weight:normal">'.$langs->trans('Cr&eacute;er  un autre fournisseur').'</a>';
                print '</form>';
        } else {
            // pas de fournisseur à afficher, ni de form
        }
	...
	if ($action != 'editfactclient' && $object->fk_soc_client && $object->statut == 0) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editfactclient&amp;id='.$object->id.'">'.img_edit($langs->trans('Modify'),1).'</a></td>';
	print '</tr></table></td><td colspan="3">';
