 ...
 */
class pdf_fiche_repa extends ModeleEquipement
{
	var $db;
	var $name;
	var $description;
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("SN")." : " . $refequipement, '', 'L');
		$posx=$this->page_largeur-$this->marge_droite-100;
		$pdf->SetXY($posx,$posy);
		...
		// Le motif de réparation dans la zone rectangulaire gauche

		// champs customtabs de l'équipement
		require_once DOL_DOCUMENT_ROOT.'/customtabs/class/customtabs.class.php';
		require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';

		$customtabs = new CustomTabs($this->db);
		$customtabs->fetch(0, 'gestionequipement');

		$objectCT = $customtabs->element_setting();
		$result = $objectCT->fetch($object->id);
			
		$extrafields = new ExtraFields($this->db);
		$extralabels=$extrafields->fetch_name_optionals_label($customtabs->table_element);

		$res=$customtabs->fetch_optionals($object->id, $extralabels);
		$value=$customtabs->array_options["options_equipretourrepa"];
		$value2=$customtabs->array_options["options_dateequiptlivraisonbelg"];

		$datebonderepa=date("d.m.y");
		...
		$pdf->MultiCell(80, 3, "REPARATION EN DATE DU : ".$datebonderepa,0,'L',0);

		$pdf->SetXY($posx+2,$posy+8);
		$pdf->SetTextColor(0,0,60);
		$pdf->SetFont('','B',$default_font_size);
		$pdf->MultiCell(80, 3, "MOTIF DE REPARATION :",0,'L',0);

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+2,$posy+13);
		$pdf->MultiCell(80, 4, $value, 0, 'L');

		// Adresse de Paris
		$adressemaaparis = "166 Avenue de Versailles - 75016 - PARIS";
		$pdf->SetXY(10,34);
		$pdf->SetTextColor(0,0,0);
		$pdf->SetFont('','',$default_font_size);
		$pdf->MultiCell(85,4,$adressemaaparis,0,'L');
	}
