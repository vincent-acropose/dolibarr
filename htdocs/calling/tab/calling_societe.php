...
dol_fiche_head($head, 'calling_societe', $langs->trans("ThirdParty"),0,'company');
$form=new Form($db);
...
// 	print_liste_field_titre($langs->trans("DoneBy"),$_SERVER["PHP_SELF"],"ud.login",$param,"","",$sortfield,$sortorder);
	...
	while ($i < min($num,$limit))
	{
		...
		print $actionstatic->getNomUrl(1,50);
		print '</td>';
		...
		if ($obj->socid)
		{
			...
			print $societestatic->getNomUrl(1,'',50);
		}
		else print '&nbsp;';
		print '</td>';
		...
		if ($obj->fk_contact > 0)
		{
			...
			print $contactstatic->getNomUrl(1,'',50);
		}
		else
		{
			print "&nbsp;";
