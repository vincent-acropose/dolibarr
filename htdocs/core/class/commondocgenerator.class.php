...
abstract class CommonDocGenerator
{
    ...
    function get_substitutionarray_user($user,$outputlangs)
    {
        ...
        return array(
            ...
            'myuser_zip'            => (!empty($user->zip) ? $user->zip : ''),
            'myuser_town'           => (!empty($user->town) ? $user->town : ''),
            'myuser_country'        => (!empty($user->country) ? $user->country : ''),
            'myuser_country_code'   => (!empty($user->country_code) ? $user->country_code : ''),
            'myuser_state'          => (!empty($user->state) ? $user->state : ''),
            'myuser_state_code'     => (!empty($user->state_code) ? $user->state_code : ''),
            'myuser_fax'            => (!empty($user->office_fax) ? $user->office_fax : ''),
            'myuser_mobile'         => (!empty($user->user_mobile) ? $user->user_mobile : ''),
            'myuser_email'          => (!empty($user->email) ? $user->email : ''),
            'myuser_logo'           => (!empty($user->photo) ? $user->photo : ''),
    ...
    function get_substitutionarray_thirdparty($object,$outputlangs)
    {
        ...
        $array_thirdparty = array(
            ...
            'company_default_bank_iban' =>(!empty($object->bank_account->iban) ? $object->bank_account->iban : ''),
            'company_default_bank_bic'  =>(!empty($object->bank_account->bic)  ? $object->bank_account->bic : '')
        );
    ...
    function get_substitutionarray_object($object,$outputlangs,$array_key='object')
    {
        ...
        $array_key.'_ref_ext'                 => (!empty($object->ref_ext) ? $object->ref_ext : ''),
        $array_key.'_ref_customer'            => $object->ref_client,
        $array_key.'_ref_supplier'            => (!empty($object->ref_fournisseur)?$object->ref_fournisseur:''),
        $array_key.'_source_invoice_ref'      => (!empty($invoice_source->ref) ? $invoice_source->ref : ''),
