...
require_once DOL_DOCUMENT_ROOT .'/core/class/original.html.formmail.class.php';
...
var_dump($this->withbody); 
        	...
        	if (! empty($this->withbody))
        	{
        		...
        		$out.= '<tr>';
        		$out.= '<td width="180" valign="top">'.$langs->trans("MailText").'<br /><br />';
                ...
                foreach (scandir(dol_buildpath($dir)) as $key => $value) {
                    if (!in_array($value, array(".", ".."))) {
                        if (!is_dir($dir . DIRECTORY_SEPARATOR . $value)) {
                            $out.= '<button type="button" class="InsertModel fullInputs" url="' . dol_buildpath($dir, 2) . $value . '">' . str_replace(array('_', '.html'), ' ', $value) . '</button>';
                        }
                    }
								}
            $out.= '<script>
                    $(\'.InsertModel\').click(function () {
                        ...
                        $.ajax({
                            ...
                            if (extension == "txt") {
                                CKEDITOR.instances.message.insertText(msg);
                            } else if (extension == "html" || extension == "htm") {
                                CKEDITOR.instances.message.insertHtml(msg);
                            }
                        })
