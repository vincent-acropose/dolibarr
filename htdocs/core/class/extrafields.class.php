...
class ExtraFields
{
	...
	'checkboxlist' => 'ExtrafieldCheckBoxList',
	...
	*/
	...
	private function create($attrname, $type='varchar', $length=255, $elementtype='member', $unique=0, $required=0, $default_value='',$param='')
	{
		...
		if (! empty($attrname) && preg_match("/^\w[a-zA-Z0-9_]*$/",$attrname) && ! is_numeric($attrname))
		{
			...
			elseif($type=='price') {
				...
				$typedb='varchar';
				$lengthdb='128';
			} elseif (($type=='select') || ($type=='sellist') || ($type=='radio') || ($type=='checkbox') || ($type=='checkboxlist')){
				$typedb='text';
				$lengthdb='';
			} elseif ($type=='link') {
				$typedb='int';
	...
	function showInputField($key,$value,$moreparam='',$keyprefix='')
	{
		...
		elseif ($type == 'hidden')
		{
			$out='<input type="hidden" class="flat" name="options_'.$key.$keyprefix.'" size="'.$showsize.'" maxlength="'.$size.'" value="'.$value.'"'.($moreparam?$moreparam:'').'>';
		}
		elseif ($type == 'text')
		{
		...
		elseif ($type == 'link')
		{
			...
			$out='<input type="text" class="flat" name="options_'.$key.$keyprefix.'"  size="20" value="'.$ref.'" >';
