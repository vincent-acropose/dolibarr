 ...
 */
class mod_barcode_product_volition extends ModeleNumRefBarCode
{
	var $nom='Volition BarCode';				// Model Name
	...
	{
		global $db,$conf;
		dol_include_once('/core/lib/barcode.lib.php');

		// EAN 13 : racine (6-8 chiffres) + numéro article (4-5 chiffres) + clé (13ème chiffre)
		...
		if (!empty($obj)) {
			if (strlen($obj->barcode) == 13) {
				$last_barcode = substr($obj->barcode, 0, -1);
			
				// Numéro article + clé
				$num_article = str_replace($prefix, '', $last_barcode);
	
				// Numéro article
				$new_num_article = $num_article + 10;
				
				$barcode = $prefix . $new_num_article;
				
				$ean_sum = barcode_gen_ean_sum($barcode);
