 ...
 */
class pdf_bl_nonchiffre extends ModelePdfExpedition
{
	...
	function __construct($db=0)
	{
		...
		$this->db = $db;
		$this->name = "BL non chiffré";
		$this->description = "Modèle de BL non chiffré";
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if(!empty($object->array_options['options_nb_colis'])) {
					...
					$object->note_public .= (!empty($object->note_public)) ? "\n" : '';
					$object->note_public .= $colis;
				}
