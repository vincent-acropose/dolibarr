...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_salaisoncvolcr2.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from salaisoncvolcr2 model
 */
...
dol_include_once("/custom/ultimatepdf/lib/ultimatepdf.lib.php");

	

/**
 *	Class to manage PDF invoice template salaisoncvolcr2
 */
class pdf_salaisoncvolcr2 extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "salaisoncvolcr2";
		$this->description = $langs->trans('PDFsalaisoncvolcr2Description');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				//** modifiaction PR
				//****************************
				// cvoLigne va contenir le total de la CVO de chacune des lignes
				//cvoRate est le taux
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// la cvo est calculé sur la quantité
					// avec la consante pdf_salaisoncvolcr2::CVORATE
					$this->cvoLigne+=round(floatval($qty)*pdf_salaisoncvolcr2::CVORATE,2);
					...
					if ($object->lines[$i]->remise_percent)
					{
                        //@pr je pense qu'il faut que la remise totale soit la somme des deux, mais en ajoutant une remise supplémentataire
                        //je la retire de la total et je peux l'inscrire en x+y mais les calculs restent juste
                        
                        
						// COnnection à la base
				
						$database = new mysqli("salaisondfdoli.mysql.db","salaisondfdoli","Choupi05","salaisondfdoli");
						
						// ecriture de la requete sql			
						$msql="SELECT remsup FROM aallx_societe_extrafields WHERE fk_object=".$object->client->id;
						$resultat=$database->query($msql);
						// j'extrais le pourcentage
						$remTab =$resultat->fetch_array(MYSQLI_NUM);
						$remSup =floatval($remTab[0]);
					    ...
					    $remisePrem  = floatval($remise_percent);
					    if ($remPrem>0 || $remPrem>$remSup || $remSup>0){
						    $remise_percent= $remisePrem-$remSup." + ".$remSup;
				...
				if ($object->mode_reglement_code == 'LCR' or $object->mode_reglement_code == 'LCRS')
				{
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				//**********************************************************
				// je place la CVO si elle n est pas nulle
				...
				if ($this->cvoLigne!=0) 

					{
						...
						$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
						$totalvat = "CVO ".(pdf_salaisoncvolcr2::CVORATE*100)." % ";
						$pdf->MultiCell($col2x-$col1x, $tab2_hl, $totalvat, 0, 'L', 1);
						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
						$pdf->MultiCell($largcol2, $tab2_hl, price($this->cvoLigne, 0, $outputlangs), 0, 'R', 1);	
					}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$pdf->MultiCell($widthrecbox, 5, "Facturer à :",0,'L');
