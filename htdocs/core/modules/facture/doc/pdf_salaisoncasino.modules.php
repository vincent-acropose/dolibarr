...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_salaisoncasino.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from salaisoncasino model
 */
...
/**
 *	Class to manage PDF invoice template salaisoncasino
 */
class pdf_salaisoncasino extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "salaisoncasino";
		$this->description = $langs->trans('PDFsalaisoncasinoDescription');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// avec la consante pdf_salaisoncasino::CVORATE
					$this->cvoLigne+=round(floatval($qty)*pdf_salaisoncasino::CVORATE,2);
			...
			else
			{
				...
				if ($this->cvoLigne!=0) 

					{
						...
						$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
						$totalvat = "CVO ".(pdf_salaisoncasino::CVORATE*100)." % ";
						$pdf->MultiCell($col2x-$col1x, $tab2_hl, $totalvat, 0, 'L', 1);
						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->thirdparty->code_client)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell($w, 3, "Notre code (V-REF) : S017705" , '', 'R');
		...
		if ($showaddress)
		{
			...
			$pdf->MultiCell($widthrecbox, 5, "Facturé à :",0,'L');
