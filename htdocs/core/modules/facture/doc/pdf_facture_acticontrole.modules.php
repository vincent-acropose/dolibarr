 ...
 */
class pdf_facture_acticontrole extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "facture_acticontrole";
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->MultiCell($this->posxdesc-$this->posxref-0.8, 3, $ref, 0, 'L');
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
					{
						...
						$pdf->MultiCell($this->posxqty-$this->posxtva-0.8, 3, $vat_rate, 0, 'R');
					}

					// Quantity
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxup-$this->posxqty-0.8, 3, $qty, 0, 'R');	// Enough for 6 chars

					// Unit price before discount
					$up_excl_tax = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxup, $curY);
					$pdf->MultiCell($this->posxttc-$this->posxup-0.8, 3, $up_excl_tax, 0, 'R', 0);
					
					// Unit price before discount
					$up_incl_tax = pdf_getlineupwithtax($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxttc, $curY);
					$pdf->MultiCell($this->posxdiscount-$this->posxttc-0.8, 3, $up_incl_tax, 0, 'R', 0);
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')
			{
				if (! empty($object->fk_bank) || ! empty($conf->global->FACTURE_RIB_NUMBER))
				{
					...
					$rib = 'RIB : '.$outputlangs->convToOutputCharset($account->domiciliation);
					$rib.= ' '.$account->code_banque;
					$rib.= ' '.$account->code_guichet;
					$rib.= ' '.$account->number;
					$rib.= ' '.$account->cle_rib;
					$pdf->MultiCell(100, 3, $rib, 0, 'L', 0);
		...
		// LOGO VERISELECT
		$logo_veriselect = $conf->mycompany->dir_output.'/logos/acticontrole-logo-veriselect.jpg';
		if (is_readable($logo_veriselect))
		{
			$height=pdf_getHeightForLogo($logo);
			$pdf->Image($logo_veriselect, $this->marge_gauche, $posy, 0, $height);	// width=0 (auto)
		}
		else
		{
			$pdf->SetTextColor(200,0,0);
			$pdf->SetFont('','B',$default_font_size - 2);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("ErrorLogoFileNotFound",$logo_veriselect), 0, 'L');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(30,2, $outputlangs->transnoentities("Ref"),'','L');
		}
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell($this->posxqty-$this->posxtva+3,2, $outputlangs->transnoentities("VAT"),'','C');
			}
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxttc-$this->posxup-1,2, $outputlangs->transnoentities("PriceUHT"),'','C');
		}
		
		$pdf->line($this->posxttc-1, $tab_top, $this->posxttc-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxttc-1, $tab_top+1);
			$pdf->MultiCell($this->posxdiscount-$this->posxttc-1,2, $outputlangs->transnoentities("PriceUTTC"),'','C');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht,2, $outputlangs->transnoentities("TotalTTC"),'','C');
		}
	}
