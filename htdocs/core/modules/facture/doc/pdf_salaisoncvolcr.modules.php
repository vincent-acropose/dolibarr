...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_salaisoncvolcr.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from salaisoncvolcr model
 */
...
/**
 *	Class to manage PDF invoice template salaisoncvolcr
 */
class pdf_salaisoncvolcr extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "salaisoncvolcr";
		$this->description = $langs->trans('PDFsalaisoncvolcrDescription');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// avec la consante pdf_salaisoncvolcr::CVORATE
					$this->cvoLigne+=round(floatval($qty)*pdf_salaisoncvolcr::CVORATE,2);
			...
			else
			{
				...
				if ($this->cvoLigne!=0) 

					{
						...
						$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
						$totalvat = "CVO ".(pdf_salaisoncvolcr::CVORATE*100)." % ";
						$pdf->MultiCell($col2x-$col1x, $tab2_hl, $totalvat, 0, 'L', 1);
						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
