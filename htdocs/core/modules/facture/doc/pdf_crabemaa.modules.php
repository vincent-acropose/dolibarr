...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_crabemaa.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from crabemaa model
 */
...
/**
 *	Class to manage PDF invoice template crabemaa
 */
class pdf_crabemaa extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "crabemaa";
		$this->description = $langs->trans('PDFcrabemaaDescription');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxtva-$curX,3,$curX,$curY,$hideref=1,$hidedesc);
	...
	function _tableau_versements(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetXY($tab3_posx+45, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Num"), 0, 'L', 0);
		$pdf->SetXY($tab3_posx+60, $tab3_top);
		$pdf->MultiCell(40, 3, $outputlangs->transnoentities("Communication"), 0, 'L', 0);
		...
		$sql = "SELECT p.datep as date, p.fk_paiement as type, p.num_paiement as num, pf.amount as amount, p.note as note,";
		$sql.= " cp.code";
		...
		if ($resql)
		{
			...
			while ($i < $num) {
				...
				$pdf->SetXY($tab3_posx+45, $tab3_top+$y);
				$pdf->MultiCell(30, 3, $row->num, 0, 'L', 0);
				$pdf->SetXY($tab3_posx+60, $tab3_top+$y);
				$pdf->MultiCell(0, 3, $row->note, 0, 'L', 0);
