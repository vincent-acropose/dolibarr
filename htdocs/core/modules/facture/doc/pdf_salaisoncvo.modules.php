...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_salaisoncvo.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from salaisoncvo model
 */
...
/**
 *	Class to manage PDF invoice template salaisoncvo
 */
class pdf_salaisoncvo extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "salaisoncvo";
		$this->description = $langs->trans('PDFsalaisoncvoDescription');
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// avec la consante pdf_salaisoncvo::CVORATE
					$this->cvoLigne+=floatval($qty)*pdf_salaisoncvo::CVORATE;
			...
			else
			{
				...
				if ($this->cvoLigne!=0) 

					{
						...
						$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
						$totalvat = "CVO ".(pdf_salaisoncvo::CVORATE*100)." % ";
						$pdf->MultiCell($col2x-$col1x, $tab2_hl, $totalvat, 0, 'L', 1);
						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
