...
class pdf_baleine_acropose extends ModelePDFProjects
{
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "baleine_acropose";
		$this->description = $langs->trans("DocumentModelBaleineAcropose");
	...
	function write_file($object,$outputlangs)
	{
		...
		if ($conf->projet->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->AddPage();
				$pagenb++;
				$this->_pagehead($pdf, $object, 1, $outputlangs, $s);
				...
				$TContactsInternes = $object->liste_contact(-1,'internal');
				$TContactsExternes = $object->liste_contact(-1,'external');
				
				$TContacts = array_merge($TContactsInternes, $TContactsExternes);
				...
				if(!empty($TContacts)) {
					...
					foreach($TContacts as $TData) {
						...
						if($TData['source'] === 'external') {
							...
							$contactstatic->fetch($TData['id']);
							
							$soc = new Societe($db);
							$soc->fetch($TData['socid']);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		// $tab_top = Position du tableau (plus le nombre est grand, plus le tableau démarre bas)
	 ...
	 *   @return	void
	 */
	function _tableau_contacts(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $taille_separateur=45)
	{
		global $conf,$mysoc;
	 ...
	 *  @return	void
	 */
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, &$s)
	{
		...
		$posy+=6;
		$pdf->SetXY($posx,$posy);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("ThirdParty")." : " . (($s->id > 0) ? $s->name : '(aucun)'), '', 'R');
