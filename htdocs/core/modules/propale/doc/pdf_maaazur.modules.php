 ...
 */
class pdf_maaazur extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "maaazur";
		$this->description = $langs->trans('DocModelMaaAzurDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$outputlangs->load("comm2Opropal@comm2o");
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetSubject($outputlangs->transnoentities("CommercialProposalMaa"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("CommercialProposalMaa"));
...
// 			GarantisOptionExtends
			
			// Garantis
			$pdf->SetFont('','', $default_font_size - 3);
			$pdf->SetXY($this->marge_gauche, $posy);
			$titre =  $outputlangs->transnoentities("GarantisOptionExtends");
			...
			// Precision sur mutuel et code de remboursmeent
			$Soci= new Societe($this->db); 
			$Soci->fetch($object->array_options['options_mutuel_customer']); 
			...
			$pdf->SetXY($this->marge_gauche, $posy);
			$titre =  $outputlangs->transnoentities("PaymentInfoMutuel", $nomenclature->numnomenclatures, $Soci->name);
...
// 		$cury+=4;
		...
		$pdf->SetFont('','', $default_font_size - 3);
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(120, 3, $outputlangs->transnoentities("InfoCautionPart1", price($object->total_ttc - $nomenclature->intervoa , 0, $outputlangs).' €'), 0, 'L', 0);
		$cury+=3;
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(120, 3, $outputlangs->transnoentities("InfoCautionPart2"), 0, 'L', 0);
		$cury+=9;

		
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(130, 3, $outputlangs->transnoentities("PaymentMontant", price( round( ($object->total_ttc - $nomenclature->intervoa)/ (int)$object->array_options['options_nbr_caution'], 2 ) , 0, $outputlangs).' €'), 0, 'L', 0);
		$cury+=3;
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(130, 3, $outputlangs->transnoentities("NumIbanKey").': ' . $outputlangs->convToOutputCharset($account->iban), 0, 'L', 0);
		$cury+=3;
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(130, 3, $outputlangs->transnoentities("NumIbanName").': ' . $outputlangs->convToOutputCharset($this->emetteur->name), 0, 'L', 0);
		$cury+=3;
		
		$pdf->SetXY($curx, $cury);
		$pdf->MultiCell(130, 3, $outputlangs->transnoentities("Communication").': ' . $outputlangs->transnoentities("InfoCautionRappel", $object->lines[0]->libelle, $outputlangs->convToOutputCharset($object->client->name) ), 0, 'L', 0);
		$cury+=3;
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			// 	var_dump($object->array_options); 
				...
				$pdf->SetTextColor(0,0,60);
				$pdf->SetFillColor(224,224,224);
				$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("RemboursementdelaMutuel"), $useborder, 'L', 1);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price((float)$nomenclature->intervoa, 0, $outputlangs), $useborder, 'R', 1);
				...
				$pdf->SetTextColor(0,0,60);
				$pdf->SetFillColor(224,224,224);
				$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("YourRestePaye"), $useborder, 'L', 1);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $nomenclature->intervoa , 0, $outputlangs), $useborder, 'R', 1);
			}
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("CommercialProposalMaa");
