...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_loc.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele Loc
 */
 
 /** Modification et mise en commentaires 
  *  Lignes : 45,79,80,114,115,116,117,121,122,123,125,126,132,133,135,136,496-507,521-557,1293,1596,1599
  */
...
/**
 *	Class to generate PDF proposal Loc
 */
class pdf_loc extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "loc";
		$this->description = $langs->trans(' Modèle de proposition commerciale complet (logo…) pour une offre en location (absence de prix unitaire et de TVA)');
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				*/
					...
					if($conf->global->PRODUCT_USE_UNITS)
					{
						$pdf->MultiCell($this->posxup-$this->posxqty-0.8, 3, $qty, 0, 'R');
					}
					else
					{
					...
					foreach( $this->localtax1 as $localtax_type => $localtax_rate )
					{
						...
						foreach( $localtax_rate as $tvakey => $tvaval )
						{
							if ($tvakey!=0)    // On affiche pas taux 0
							{
								...
								$tvacompl='';
		enlever les "double quote"						if (preg_match('/\*"/"',$tvakey))
								{
...
*/
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			if (empty($hidetop))
			{
				...
				$pdf->MultiCell($this->posxup-$this->posxtva+3,2, $outputlangs->transnoentities(""),'','C');
			}
	...
	function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// Total HT
		$pdf->SetFillColor(255,255,255);
		$pdf->SetXY($posx, $tab_top - 20); // - 20 est le décallage en hauteur du texte
		$pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("ProposalCustomerSignature"), 0, 'L', 1);

		$pdf->SetXY($posx, $tab_top -20 + $tab_hl); // -20 est le décallage en hauteur du rectangle
