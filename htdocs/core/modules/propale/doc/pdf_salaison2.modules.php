...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_salaison2.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele salaison2
 */
...
/**
 *	Class to generate PDF proposal salaison2
 */
class pdf_salaison2 extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "salaison2";
		$this->description = $langs->trans('DocModelsalaison2Description');
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					if ($object->lines[$i]->remise_percent)
					{
						...
						$remisePrem  = floatval($remise_percent)-$remSup;
					    if ($remPrem>0 || $remPrem>$remSup || $remSup>0){
						    $remise_percent= $remisePrem." + ".$remSup;
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 6, $outputlangs->transnoentities("Ref")." : " . $outputlangs->convToOutputCharset($object->ref), '', 'R');
