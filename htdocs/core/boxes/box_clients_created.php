 ...
 */
class box_clients_created extends ModeleBoxes
{
	var $boxcode="lastcustomerscreated";
	var $boximg="object_company";
	var $boxlabel="BoxLastCustomersCreated";
	...
	function loadBox($max=10)
	{
        ...
        $this->info_box_head = array('text' => $langs->trans("BoxTitleLastCreatedCustomers",$max));

		if ($user->rights->societe->lire)
		{
			$sql = "SELECT s.nom, s.rowid as socid, s.datec, s.tms, s.status, s.client, te.code as typent_code";
			...
			$sql.= " WHERE ";
			$sql.= " s.entity IN (".getEntity('societe', 1).")";
			...
			if ($result)
			{
				...
				while ($i < $num)
				{
					...
					'text' => $thirdpartystatic->LibCustProspStatut($objp->client));

					$arr = $formcompany->typent_array(1);
					$this->info_box_contents[$i][3] = array('td' => 'align="right"',
					'text' => $arr[$objp->typent_code]);
