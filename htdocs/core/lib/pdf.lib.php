...
function pdf_getlinetotalexcltax($object,$i,$outputlangs,$hidedetails=0)
{
	...
	else
	{
		...
		{
			if (empty($hidedetails) || $hidedetails > 1) return price($sign * $object->lines[$i]->total_ht, 0, $outputlangs,1,'MT');
		}
	}
	return '';
}
...
function pdf_getlinetotalwithtax($object,$i,$outputlangs,$hidedetails=0)
{
	...
	else
	{
		...
		{
			if (empty($hidedetails) || $hidedetails > 1) return price($sign * (($object->lines[$i]->total_ht) + ($object->lines[$i]->total_ht)*($object->lines[$i]->tva_tx)/100), 0, $outputlangs,1,'MT');
		}
	}
	return '';
}
