 ...
 */
class emailgenerator extends CommonDocGenerator
{
...
// 		}
		...
		$content = file_get_contents(DOL_DOCUMENT_ROOT. $srctemplatepath ); 
...
// 				else
// 				{
					$socobject = new Societe($this->db); 
					$socobject->fetch( $object->socid ); 
...
// 				// Make substitution
				...
				$parameters=array('file'=>$srctemplatepath,'object'=>$object,'outputlangs'=>$outputlangs,'substitutionarray'=>&$substitutionarray);
...
// 					$odfHandler->setVars('free_text', $newfreetext, true, 'UTF-8');
					...
					$content = str_replace('{free_text}', $newfreetext, $content); 
...
// 					preg_match('#(.!-- BEGIN row.lines --])(.*)([!-- END row.lines --])#i', $content, $match);
					
					$deb = substr($content, strpos($content, '{!-- BEGIN row.lines --}' )+24); 
					$ctline = substr($deb,0,(strpos($deb, '{!-- END row.lines --}' )) );  
					$allline = ''; 
...
// 						$parameters=array(/*'odfHandler'=>&$ctline,*//*'file'=>$file,*/'object'=>$object,'outputlangs'=>$outputlangs,'substitutionarray'=>&$tmparray,'line'=>$line);
...
// 								$listlines->setVars($key, $val, true, 'UTF-8');
								$ctline = str_replace('{'.$key.'}', $val, $ctline); 
							}
							catch(OdfException $e)
							{
							}
...
// 					$odfHandler->mergeSegment($listlines);
					...
					$deb = substr($content,0,  strpos($content, '{!-- BEGIN row.lines --}' )); 
					
					$deb.=$allline; 
					
					$deb.= substr($content,(strpos($content, '{!-- END row.lines --}' )+24) );  
