...
class ProductFournisseur extends Product
{
    ...
    function find_min_price_product_fournisseur($prodid, $qty=0)
    {
        ...
        $sql.= " pfp.price, pfp.quantity, pfp.unitprice *(100 - pfp.remise_percent)/100 unitprice, pfp.tva_tx, pfp.charges, pfp.unitcharges";
        $sql.= " FROM ".MAIN_DB_PREFIX."societe as s, ".MAIN_DB_PREFIX."product_fournisseur_price as pfp";
        $sql.= " WHERE s.entity IN (".getEntity('societe', 1).")";
        $sql.= " AND pfp.fk_product = ".$prodid;
        $sql.= " AND pfp.fk_soc = s.rowid";
        if ($qty > 0) $sql.= " AND pfp.quantity <= ".$qty;
        $sql.= " ORDER BY pfp.unitprice *(100 - pfp.remise_percent)/100";
