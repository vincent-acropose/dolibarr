...
if ($result) {
	...
	while ( $i < $num ) {
		...
		$tabcompany [$obj->rowid] = array (
			'id' => $obj->socid,'name' => $obj->name,'client' => $obj->client 
		);
		$i ++;
	}
} else {
 ...
 */
	...
	foreach ( $tabfac as $key => $val ) {
		...
		$lines = array (
			array (
			'var' => $tabttc [$key],'label' => $langs->trans ( 'ThirdParty' ) . ' (' . $companystatic->getNomUrl ( 0, 'customer', 16 ) . ')','nomtcheck' => true,'inv' => true 
		),array (
			'var' => $tabht [$key],'label' => $langs->trans ( 'Products' ) . ' (' . $companystatic->getNomUrl ( 0, 'customer', 16 ) . ')'
		),array (
			'var' => $tabtva [$key],'label' => $langs->trans ( 'VAT' ) . ' (' . $companystatic->getNomUrl ( 0, 'customer', 16 ) . ')'
		),array (
