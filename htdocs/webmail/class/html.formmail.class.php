...
/**
 *  \file       webmail/class/html.formmail.class.php
 *  \ingroup    webmail
 *  \brief      file to generate a mail form
 */
...
class FormWebMail
{
    ...
    function get_form($addfileaction='addfile',$removefileaction='removefile')
    {
        ...
        else
        {
        	...
        	if(!$this->withto)
        	{
        		$out.= '<tr><td width="180" class="fieldrequired">';
				$out.= $langs->trans("MailTo");
        		$out.= '</td><td>';	
        		$out.= select_mail_receiver('','socid','',1);
        		$out.= "</td></tr>\n";
        	}
        	else
        	{
        		...
        		$out.= '<input readonly size="'.(is_array($this->withto)?"30":"60").'" id="sendto" name="sendto" value="'.((! is_array($this->withto) && ! is_numeric($this->withto))? (isset($_POST["sendto"])?$_POST["sendto"]:$this->withto) : (isset($_POST["sendto"])?$_POST["sendto"]:"") ).'" />';
        	...
        	if (! empty($this->withtoccc) || is_array($this->withtoccc))
        	{
        		...
        		{
        			$out.= select_mail_receiver('','socidcc','',1,true);
        			$out.= '<br><input size="'.(is_array($this->withtoccc)?"30":"120").'" id="sendtoccc" name="sendtoccc" value="'.($this->withtoccc!=1?$this->withtoccc:"").'" />';
			...
			// BCC
        	if (! empty($this->withtobcc) || is_array($this->withtobcc))
        	{
        		$out.= '<tr><td width="180">';
        		$out.= $form->textwithpicto($langs->trans("MailBCC"),$langs->trans("YouCanUseCommaSeparatorForSeveralRecipients"));
        		$out.= '</td><td>';
        		if (! empty($this->withtobccreadonly))
        		{
        			$out.= (! is_array($this->withtobcc) && ! is_numeric($this->withtobcc))?$this->withtobcc:"";
        		}
        		else
        		{
        			$out.= '<input size="'.(is_array($this->withtobcc)?"30":"60").'" id="sendtobcc" name="sendtobcc" value="'.((! is_array($this->withtobcc) && ! is_numeric($this->withtobcc))? (isset($_POST["sendtobcc"])?$_POST["sendtobcc"]:$this->withtobcc) : (isset($_POST["sendtobcc"])?$_POST["sendtobcc"]:"") ).'" />';
        			if (! empty($this->withtobcc) && is_array($this->withtobcc))
        			{
        				$out.= " ".$langs->trans("or")." ";
        				$out.= $form->selectarray("receiverccc", $this->withtobcc, GETPOST("receiverbcc"), 1);
        			}
        		}
        		//if (! empty($conf->global->MAIN_MAIL_AUTOCOPY_TO)) print ' '.info_admin("+ ".$conf->global->MAIN_MAIL_AUTOCOPY_TO,1);
        		$out.= "</td></tr>\n";
        	...
        	if (! empty($this->withtopic))
        	{
        		...
        		$out.= '<tr>';
        		$out.= '<td width="180" class="fieldrequired">'.$langs->trans("MailTopic").'</td>';
        		$out.= '<td>';
