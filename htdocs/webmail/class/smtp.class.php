...
class SMTPFile
{
	...
	function sendfile()
	{
		...
		if (empty($conf->global->MAIN_DISABLE_ALL_MAILS))
		{
			...
			// If we use SSL/TLS
			$server=$conf->global->WEBMAIL_SMTP_SERVER;
			if (! empty($conf->global->WEBMAIL_EMAIL_TLS) && function_exists('openssl_open')) $server='ssl://'.$server;

			$this->smtps->setHost($server);
			$this->smtps->setPort($conf->global->WEBMAIL_SMTP_PORT); // 25, 465...;
			
			
			dol_include_once("/webmail/class/userconfig.class.php");
			
			$mailconfig = new Userconfig($db);
			$mailconfig->fetch_from_user($user->id);

			$this->smtps->setID($mailconfig->login);
			$this->smtps->setPW($mailconfig->password);
			...
			if (! $from)
			{
				$this->error="Failed to send mail with smtps lib to HOST=".$server.", PORT=".$conf->global->WEBMAIL_SMTP_PORT."<br>Sender address '$from' invalid";
			...
			if (! $dest)
			{
				$this->error="Failed to send mail with smtps lib to HOST=".$server.", PORT=".$conf->global->WEBMAIL_SMTP_PORT."<br>Recipient address '$dest' invalid";
				dol_syslog("CMailFile::sendfile: mail end error=".$this->error, LOG_ERR);
				$res=false;
			}
