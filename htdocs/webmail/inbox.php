...
if ($action=='GetNewMessages')
{
	...
	if (empty($conf->global->WEBMAIL_SAFE_SERVER))
	{
		if($userpop->safemail==1)
		{
			$pop3->delete_old_messages($userpop->dayssafe);
		}
		else
		{
			$pop3->delete_old_messages($userpop->dayssafe,true);
		}
	}
	
	$pop3->close();
}
if ($action == 'delete' && $user->rights->webmail->delete)
{
	if (is_array($toDelete))
	{
	    $object = new Message($db);
		$result = $object->multidelete($user,$toDelete);	  
		...
		if ($result < 0)
    	{
   		...
   		{
   			setEventMessage($langs->trans("MailsDeleted",$result));
   		}
	}
}
...
$sql = "SELECT rowid, `from` as envia, `subject` as asunto, datetime as datec, state_new as leido, state_reply, state_spam, files";
$sql.= " FROM ".MAIN_DB_PREFIX."webmail_mail";

$sql.= " WHERE fk_user IN ".getusersmail()." AND is_outbox=0";
$sql.= " AND entity=".$conf->entity;


if($archiv)
{
	$sql.= " AND state_archiv=1";
}
else
{
	$sql.= " AND state_archiv=0";
}
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans('MailSender'),$_SERVER["PHP_SELF"],'from','',$param,'width="25%"',$sortfield,$sortorder);
	...
	while ($i < min($num,$limit))
	{
		...
		//Remitente
		print '<td class="nobordernopadding nowrap" width="25%">';
		...
		print '<td align="center">';
		print '<input id="'.$i.'" class="flat checkformail" type="checkbox" name="toDelete[]" value="'.$objp->rowid.'">';
	...
	{
		print '<input type="submit" class="button" value="'.$langs->trans("DeleteSelected").'">';
	}
	print "</div>";	
	print '<br>';
	print '</form>';
