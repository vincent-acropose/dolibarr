...
print '</td>';
print '<td>'.$langs->trans("Ref").'</td><td><input type=text size=10 name=refelement id=refelement value="'.GETPOST("refelement").'"></td>';
...
if ($result)
{
	...
	if ($num) {
		...
		while ($i < min($num,$limit))
		{
			...
			print '<td align=right><a href="listelink.php?rowid='.$objp->rowid.'&action=delete">'.img_delete().'</a></td>';
			print "</tr>\n";
			$i++;
		}
	}
