...
if ($action == 'confirm_paiement' && $confirm == 'yes')
{
    ...
    if (! $error)
    {
        ...
        else
        {
			...
			$sql= "UPDATE llx_bank_url SET url_id=".$id.", label='".$object->nom."'";
...
if ($action == '' || $action == 'confirm_paiement' || $action == 'add_paiement')
{
		...
		$sql.= ' AND f.type IN (0,1,3)';	// Standard invoice, replacement, deposit
		$sql.= ' AND f.fk_soc = s.rowid';

		// on récupère la liste des clients liés à ce tiers
		$sqlTiers = "SELECT ec.element_id as othertiers from ".MAIN_DB_PREFIX."element_contact as ec, ".MAIN_DB_PREFIX."socpeople as sp";
		...
		if ($resqlTiers)
		{
			...
			if (strlen($szlistTiers) > 2)
				$sql.= ' AND f.fk_soc in ('.substr($szlistTiers,0,-2).')';
			else // si on a rien on bloque le filtre
				$sql.= ' AND 1=2';
		}
...
if (! GETPOST('action'))
{
    ...
    $sql.= " , ".MAIN_DB_PREFIX."societe as s,  ".MAIN_DB_PREFIX."element_contact as ec , ".MAIN_DB_PREFIX."socpeople as sp";
    $sql.= ' WHERE p.fk_facture = f.rowid AND p.fk_paiement = c.id';
    $sql.= ' AND f.entity = '.$conf->entity;
    $sql.= ' AND f.fk_soc = s.rowid';
    if ($id)
    {
        $sql.= ' AND ((f.fk_soc = '.$id.') or  (sp.rowid= ec.fk_socpeople and ec.element_id = s.rowid and ec.fk_c_type_contact=192 and sp.fk_soc='.$id.'))';
    }
//print $sql;
