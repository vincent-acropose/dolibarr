...
// 		exit; 
			$mag = GETPOST('magasin');
			$magtext = GETPOST('magasintext');
			$client = GETPOST('client');
			$clienttext = GETPOST('clienttext');
			$ref = GETPOST('equipement');
			$rowidsel = GETPOST('rowid');

			$res = explode(';', $sqlQuerydo); 
// var_dump($res); 
			foreach($res as $sql) {
				if(strlen($sql) > 2){
						$c = $m = false; 
					...
					if( preg_match('/#CLIENT#/i',$sql1) ) {
						
						if (preg_match('#^[0-9]*$#i',$client) )
							$c = $client ;
						elseif( preg_match('#([0-9]*)#i',$clienttext, $match) && !empty($match[1]) ) {
							$c = $match[1];
						}
					
						if(!$c) 
							$sql=str_replace("e.fk_soc_client = '#CLIENT#',", '',$sql); 

					}
					
					
					if( preg_match('/#MAG#/i',$sql1) ) {
					
						if (preg_match('#^([0-9]*)$#i',trim($mag)) )
							$m = $mag ;
						elseif( preg_match('#([0-9]*)#i',$magtext, $matc) && !empty($matc[1]) ) {
							$m = $matc[1];
						}

						if(!$m) 
							$sql=str_replace("magcibleeqpt = '#MAG#',", '',$sql); 

					}

					$sqlQueryarray[] = str_replace(array("#ROWID#", '#MAG#','#CLIENT#', '#EQUIPEM#'), array($rowidsel, $m,$c, $ref), $sql);

				}
			}
/*			
			print_r($sqlQueryarray); 
			
			exit; */
			
		if(count($sqlQueryarray)> 0 )
			foreach($sqlQueryarray as $sql) {
					dol_syslog("mylist.php"."::sqlQuerydo=".$sql);
					$resultdo=$db->query($sql);
// 					var_dump($resultdo); 
			}
...
// 	exit; 
// 	header('location: mylist.php?code='.GETPOST('code').'&idmenu='.GETPOST('idmenu'));
		return true; 

}elseif (GETPOST('action')=="dojob"){
...
// 						var_dump(GETPOST('code')); 
// 						exit;
		if(GETPOST('code') == 'receptioncolis1') {
			$mag = GETPOST('magasin');
			$magtext = GETPOST('magasintext');
			$client = GETPOST('client');
			$clienttext = GETPOST('clienttext');
			$ref = GETPOST('equiement');
			...
			foreach($res as $sql) {
				if(strlen($sql) > 2){
					...
					if( preg_match('/#CLIENT#/i',$sql1) ) {
						...
						if (preg_match('#^[0-9]*$#i',$client[$rowidsel]) )
							$c = $client[$rowidsel] ;
						elseif( preg_match('#([0-9]*)#i',$clienttext[$rowidsel], $match) && !empty($match[1]) ) {
							$c = $match[1];
						}
					...
					if( preg_match('/#MAG#/i',$sql1) ) {
						...
						if (preg_match('#^([0-9]*)$#i',trim($mag[$rowidsel])) )
							$m = $mag[$rowidsel] ;
						elseif( preg_match('#([0-9]*)#i',$magtext[$rowidsel], $matc) && !empty($matc[1]) ) {
							$m = $matc[1];
						}
					...
					$sqlQueryarray[] = str_replace(array("#ROWID#", '#MAG#','#CLIENT#', '#EQUIPEM#'), array($rowidsel, $m,$c, $ref[$rowidsel]), $sql);

				}
			}
		}

		elseif(GETPOST('code') == 'facturehaddad1') {
			$mag = GETPOST('magasin');
			$magtext = GETPOST('magasintext');
			$client = GETPOST('client');
			$clienttext = GETPOST('clienttext');
...
function stockthisrow( valueref){

// 	var ref = valueref; 
// 	 console.log(  ref );
	$.get( "<?php echo DOL_URL_ROOT .'/mylist/mylist.php?action=ajaxrow&code=recepetioncolis0';  ?> ", { 
		code: 'receptioncolis0' ,
		rowid: valueref ,
		magasin: $("#magasin_"+valueref+""). val() ,
		magasintext: $("#magasin_"+valueref+""). val() , 
		client: $("#client_"+valueref+""). val() ,
		clienttext: $("#client_"+valueref+""). val() ,
		equipement : $("#equiement_"+valueref+""). val()
		} )
  .done(function( data ) {

							
				$('#button'+valueref).prev().attr('checked', false);
				 $('#button'+valueref).parent().parent().each(function(){
						$( this ).removeClass( "selected" );
						$( this ).children('td').each(function(elem) {
							if(elem ==0) {
								var text = $( '#magasintext_'+valueref+'').val();
								$( this ).html(text);
							}
							else if(elem ==2) {
								var text = $( '#clienttext_'+valueref+'').val();
								$( this ).html(text);
							}
							else if(elem ==3) {
								var text = $( '#equiement_'+valueref+'').val();
								$( this ).html(text);
							}
						}); 
				});
				$('#button'+valueref).remove(); 
...
$( document ).ready(function() {
  ...
  <?php if(GETPOST('code') == 'receptioncolis1'): ?>
	$( "input[type=checkbox]" ).each(function() {
		$( this ).change(function(){
			if( $( this ).is(':checked') ) {
				var valueref = $( this ).val();
				$( this ).parent().parent().each(function(){
						$( this ).addClass( "selected" );
						$( this ).children('td').each(function(elem) {
							if(elem ==0) {
								var text = $( this ).text();
								$( this ).html('<input type="hidden" class="selecteurmagasin"  id="magasin_'+valueref+'"  name="magasin['+valueref+']" value="'+text+'" /><input type="text" id="magasintext_'+valueref+'" class="ui-autocomplete-input selecteurmagasintext"  name="magasintext['+valueref+']" value="'+text+'"  autocomplete="off"/><ul class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"></ul>');
							}
							else if(elem ==2) {
								var text = $( this ).text();
								$( this ).html('<input type="hidden" class="selecteurclient" id="client_'+valueref+'" name="client['+valueref+']" value="'+text+'" /><input type="text" id="clienttext_'+valueref+'" class="ui-autocomplete-input selecteurclienttext"  name="clienttext['+valueref+']" value="'+text+'" autocomplete="off"/><ul class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"></ul>');
							}
							else if(elem ==3) {
								var text = $( this ).text();
								$( this ).html('<input type="text" id="equiement_'+valueref+'" name="equiement['+valueref+']" value="'+text+'" />');
							}
						}); 
				});
					...
					$( ".selecteurmagasintext" ).autocomplete({
						...
						select: function( event, ui ) {
								$( ".selecteurmagasintext" ).prev().val( ui.item['fk_magid'] ); 
						},
						open: function() {
					...
					$( ".selecteurclienttext" ).autocomplete({
						...
						minLength: 3,
						select: function( event, ui ) {
								$( ".selecteurclienttext" ).prev().val( ui.item['fk_socid'] ); 
						},
						open: function() {
							$( this ).removeClass( "ui-corner-all" ).addClass( "ui-corner-top" );
						},
	...
	<?php elseif(GETPOST('code') == 'receptioncolis0'): ?>
...
// 						$( this ).after().append( "<input  type=\"submit\" value=\"Valide\" onclick=\"\" /></form>" );
						$( this ).children('td').each(function(elem) {
							if(elem ==0) {
								var text = $( this ).text();
								$( this ).html('<input type="hidden" class="selecteurmagasin" id="magasin_'+valueref+'"   name="magasin['+valueref+']" value="'+text+'" /><input type="text" id="magasintext_'+valueref+'" class="ui-autocomplete-input selecteurmagasintext"  name="magasintext['+valueref+']" value="'+text+'"  autocomplete="off"/><ul class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"></ul>');
								...
								$( this ).append( "<input id=\"button"+valueref+"\" type=\"button\" value=\"Valide\" onclick=\"stockthisrow( '"+valueref+"')\" />" );
							}
  ...
  <?php elseif(GETPOST('code') == 'facturehaddad1'): ?>
	$( "input[type=checkbox]" ).each(function() {
		$( this ).change(function(){
			if( $( this ).is(':checked') ) {
				var valueref = $( this ).val();
				$( this ).parent().parent().each(function(){
						...
						$( this ).children('td').each(function(elem) {
							if(elem ==0) {
								var text = $( this ).text();
								$( this ).html('<input type="hidden" class="selecteurmagasin" name="magasin['+valueref+']" value="'+text+'" /><input type="text" class="ui-autocomplete-input selecteurmagasintext"  id="magasintext_'+valueref+'" name="magasintext['+valueref+']" value="'+text+'"  autocomplete="off"/><ul class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"></ul>');
							}
							else if(elem ==2) {
								var text = $( this ).text();
								$( this ).html('<input type="hidden" class="selecteurclient" name="client['+valueref+']" value="'+text+'" /><input type="text" class="ui-autocomplete-input selecteurclienttext"  id="clienttext_'+valueref+'" name="clienttext['+valueref+']" value="'+text+'" autocomplete="off"/><ul class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all"></ul>');
							}
							else if(elem ==4) {
								var text = $( this ).text();
								$( this ).html('<input type="text"  id="equiement_'+valueref+'" name="equiement['+valueref+']" value="'+text+'" />');
...
if ($result)
{
	...
	if (! empty($conf->global->MAIN_USE_JQUERY_DATATABLES))
	{
		...
		{	// on récupère le champ servant de clé pour la ligne
			...
			print '<form method="GET" action="'.$_SERVER["PHP_SELF"].'" id="querydo">';
	...
	if (! empty($myliststatic->querydo))
	{
		...
		print '<input id="goaction" class="butAction" type="button" value="'.$langs->trans('DoJob').'" >';
		print "</div>";
