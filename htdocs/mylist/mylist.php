		...
		foreach ($ArrayTable as $key => $fields) 
		{
			if (!empty($conf->global->MAIN_USE_JQUERY_DATATABLES) || $fields['visible']=='1')
			{
				...
				elseif ((
					...
					strpos($fields['field'], 'fk_') > 0 		) && $fields['param'])
				{
					// pour les clés qui sont lié à un autre élément
					print '<td nowrap="nowrap" align="'.$fields['align'].'">';
					...
					{
						case 3:
							$sql = 'SELECT '.$tblelement[1].' as rowid, '.$tblelement[2].' as label';
							$sql.= ' FROM '.MAIN_DB_PREFIX .$tblelement[0];
							$sql.= ' WHERE '.$tblelement[1].'='.$objp->$fieldsname;
							$resqlf = $db->query($sql);
