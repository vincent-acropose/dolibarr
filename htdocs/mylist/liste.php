...
if ($lists != -1)
{
	...
	print '<th width=50px>'.$langs->trans("Fields").'</th>';
	print '</tr>';
	print '</thead>';
	print '<tbody>';
	$var=true;
	foreach ($lists as $list)
	{
		...
		print "<tr ".$bc[$var].">\n";
		print "\t<td><a href='fiche.php?code=".$list['code']."'>".$list['code']."</a></td>\n";
		print "\t<td><a href='fiche.php?rowid=".$list['rowid']."'>".$list['label']."</a></td>\n";
...
{
	print '<a class="butAction" href="fiche.php?action=create">'.$langs->trans('NewList').'</a>';
}
else
{
