...
if ($action == 'add' && $user->rights->mylist->creer)
{
	...
	if (empty($_POST["code"]))
	{
		$mesg.='<div class="error">'.$langs->trans("ErrorFieldRequired",$langs->transnoentities("code")).'</div>';
		$error++;
	}
	...
	if (! $error)
	{
		...
		if (! $error)
		{
			header("Location:fiche.php?code=".$object->code);
			exit;
		}
		else
			$action = 'create';
...
if ($action == 'create' && $user->rights->mylist->creer)
{
	...
	// Code
	print '<tr><td><span class="fieldrequired">'.$langs->trans("Code").'</span></td><td><input size="12" type="text" name="code" value="'.$_POST["code"].'"></td></tr>';
...
elseif ($action == 'importexport' && $user->rights->mylist->creer)
{
	...
	if($code)
		print $object->getexporttable($code);
...
elseif ($action == 'update' && $user->rights->mylist->creer)
{
	...
	print '<tr><td><span class="fieldrequired">'.$langs->trans("Code").'</span></td><td><input size="12" type="text" name="code" value="'.$code.'"></td></tr>';
...
else
{
	...
	// Code
	print '<tr><td width="30%">'.$langs->trans("Code").'</td><td>';
	...
	if ($user->rights->mylist->creer)
		print '<a class="butAction" href="fiche.php?code='.$object->code.'&rowid='.$object->rowid.'&action=update">'.$langs->trans('Update').'</a>';
	else
		print '<a class="butActionRefused" href="#" title="'.$langs->trans("NotAllowed").'">'.$langs->trans('Update').'</a>';

	if ($user->rights->mylist->creer)
		print '<a class="butAction" href="fiche.php?code='.$object->code.'&action=importexport">'.$langs->trans('ImportExport').'</a>';
	else
	...
	if ($user->rights->mylist->supprimer && !$object->active)
		print '<a class="butAction" href="fiche.php?code='.$object->code.'&action=delete">'.$langs->trans('Delete').'</a>';
	else
	...
	if(is_array($object->listsUsed))
	{
		...
		foreach ($object->listsUsed as $key=> $value )
		{
			...
			print '<td><a href=champ.php?code='.$code.'&key='.urlencode($key).'>'.($value['field']? $value['field']:$langs->trans('empty')).'</a></td>' ;
			print '<td>'.$value['name'].'</td>' ;
			print '<td>'.$langs->trans($value['name']).'</td>' ;
			print '<td>'.$value['alias'].'</td>' ;
			print '<td>'.$value['type'].'</td>' ;
			print '<td>'.$value['elementfield'].'</td>' ;
	...
	if ($user->rights->mylist->creer)
		print '<a class="butAction" href="champ.php?code='.$object->code.'&action=create'.$param.'&backtopage='.urlencode($_SERVER['PHP_SELF'].'?code='.$code).'">'.$langs->trans('AddField').'</a>';
	else
		print '<a class="butActionRefused" href="#" title="'.$langs->trans("NotOwnerOfProject").'">'.$langs->trans('AddField').'</a>';
