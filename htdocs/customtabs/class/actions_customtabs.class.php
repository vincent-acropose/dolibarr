...
class ActionsCustomtabs // extends CommonObject 
{
	...
	function beforeODTSave($parameters, $object, $action) 
	{
		...
		// variables de limitation des contacts à récupérer
		$nbLineOnODT=$conf->global->nbLineOnODT;

		$tbltabs = $customtabs->liste_array($elementselect, 2); // on gère ici les onglets de type listes sont gérées ailleurs
		if (count($tbltabs ) >0 )
		{
			...
			{	
				...
				if ($resql)
				{
					try
					{
						...
						// pour limiter si demandé les enregistrements
						...
						if ($num)
						{
							...
							while ($i < $num)
							{
								...
								foreach($extrafields->attribute_type as $key => $value)
								{
									...
									catch(OdfException $e) {} // pour désactiver cette erreur
									catch(SegmentException $e) {} // pour désactiver cette erreur
								}
								$listlines->merge();
								$i++;
							}
