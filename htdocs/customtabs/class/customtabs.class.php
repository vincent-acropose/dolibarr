	...
	function update($user)
	{
		...
		if ($result)
		{
			...
			if ( $this->fk_statut == 1 && $this->fk_parent < 0 )
			{
				...
				switch($this->mode) {
					case 1 :	
						$tabinfo=$contacttab.':+customtabs_'.$this->rowid.':'.$this->libelle.':@customtabs:/customtabs/tabs/card.php?tabsid='.$this->rowid.'&id=__ID__';
						break;
					case 2 :
						$tabinfo=$contacttab.':+customtabs_'.$this->rowid.':'.$this->libelle.':@customtabs:/customtabs/tabs/list.php?tabsid='.$this->rowid.'&id=__ID__';
						break;
				}
...
//print $sql.'<br>';
	 ...
	 *		@param		integer		$filtermode			mode of element to selec (0= all, 1=card, 2=list)
	 *		@return		array							String with URL
	 */
	function liste_array($elementname='', $filtermode=0)
	{
		global $conf,$langs;

		$projets = array();

		$sql = "SELECT rowid, libelle, mode, fk_parent, element, tablename ";
	...
	function getShowCustomtabs($fk_user)
	{
		...
		$resql=$this->db->query($sql);
		if ($resql)
		{	// si pas de groupe, c'est ouvert à tous
...
//exit;
	...
	function getexporttable($id)
	{
		...
		$tmp.="<libelle>".$this->label."</libelle>\n";
		...
		if ($result)
		{
			...
			foreach($tblsheets as $sheet)
			{
				...
				if (! $error)
				{
				    ...
				    if (isset( $sheet["attrname"] ) && preg_match("/^\w[a-zA-Z0-9-_]*$/", $sheet["attrname"]))
					{
						$result=$extrafields->update($sheet["attrname"], $sheet["label"], $sheet["type"], $extrasize, $elementtype, ($sheet["unique"]?1:0), ($sheet["required"]?1:0), $pos, $params);
						if ($result <= 0)
						{
							$error++;
