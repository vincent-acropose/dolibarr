...
function customtabs_completesubstitutionarray(&$substitutionarray, $langs, $object)
{
	...
	// on récupère les infos de la société
	$elementselect='thirdparty';
	$tbltabs = $customtabs->liste_array($elementselect, 1 ); // on ne sélectionne que les onglets de type fiche, les listes sont gérées ailleurs
	if (count($tbltabs ) >0 )
	{
	...
	if (count($tbltabs ) >0 )
	{
		...
		{	//var_dump($customtabsarray);
			...
			if ($resql)
			{
				...
				{
					// si pas de ligne d'extrafields (non renseigné)
					// on remplace les zones par des emplacements vide
					$extrafields->fetch_name_optionals_label($elementtype);
					foreach($extrafields->attribute_type as $key => $value)
					{
						// pour afficher les zones à vide
						$odtfieldname=$elementtype."_".$key;
