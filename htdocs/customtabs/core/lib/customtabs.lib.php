...
function elementarray()
{
	...
	// on gère selon l'activation ou non du module
	...
	if (! empty($conf->propal->enabled))		$arrayelement['propal'] 			= $langs->trans("Proposal");
	if (! empty($conf->commande->enabled))		$arrayelement['commande']			= $langs->trans("Order");
	if (! empty($conf->fournisseur->enabled))	$arrayelement['supplier_order'] 	= $langs->trans("SupplierOrder");
	if (! empty($conf->facture->enabled))		$arrayelement['invoice'] 			= $langs->trans("Invoice");	
	if (! empty($conf->fournisseur->enabled))	$arrayelement['supplier_invoice'] 	= $langs->trans("SupplierInvoice");	
	$arrayelement['project'] 			= $langs->trans("Project");
	if (! empty($conf->contrat->enabled))		$arrayelement['contract'] 			= $langs->trans("Contract");
	if (! empty($conf->ficheinter->enabled))	$arrayelement['intervention'] 			= $langs->trans("Intervention");
	if (! empty($conf->banque->enabled))	$arrayelement['bank'] 			= $langs->trans("Account");
	...
	if (! empty($conf->salaries->enabled))	$arrayelement['payment_salaries'] 	= $langs->trans("SalaryPayment");
...
function getmodelib($modetab)
{
	...
	if ($modetab == 1) { return $langs->trans("Fiche"); }
	if ($modetab == 2) { return $langs->trans("Liste"); }
}
