...
/**
 * \file htdocs/customtabs/categorie.php
 * \ingroup category
...
if ($customtabs->element == 'product')
	$type = 0;
if ($customtabs->element == 'societe')
	$type = 2;
if ($customtabs->element == 'member')
	$type = 3;
if ($customtabs->element == 'contact')
	$type = 4;
	...
	if (! empty($conf->global->SOCIETE_USEPREFIX)) // Old not used prefix field
{
	...
	if ($soc->client)
		formCategory($db, $soc, 2, $socid, $user->rights->societe->creer);
	
	if ($soc->client && $soc->fournisseur)
		print '<br><br>';
	
	if ($soc->fournisseur)
		formCategory($db, $soc, 1, $socid, $user->rights->societe->creer);
} else if ($id || $ref) {
	if ($type == 0) {
		$langs->load("products");
