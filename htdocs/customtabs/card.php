...
if (! $sortfield) {  $sortfield="c.tablename"; }
...
if ($action == 'add' && $user->rights->customtabs->configurer)
{
	...
	if ($action == 'add')
	{
		...
		{
			$customtabs->libelle     = trim($libelle);				// non de l'onglet
			...
			{	// la saisie du nom de la table est obligatoire sinon on ne crée pas la table
				...
				'tms' => array('type'=>'timestamp','attribute'=>'on update CURRENT_TIMESTAMP','default'=>'CURRENT_TIMESTAMP','null'=>'not null','extra'=> 'ON UPDATE CURRENT_TIMESTAMP'),
				'fk_element' => array('type'=>'int','value'=>'11','null'=>'not null'),  // clé de l'élément 
				'fk_customtabs_parent' => array('type'=>'int','value'=>'11','null'=>'not null'),
...
if ($action == 'update' && $user->rights->customtabs->configurer)
{
	...
	$customtabs->libelle		= trim($_POST["libelle"]);
...
if (! $rowid && $action != 'create' && $action != 'import' && $action != 'edit')
{
	$customtabs = new Customtabs($db);
	...
	$var=True;
	if (count($customtabs->liste_array()) >0)
		foreach($customtabs->liste_array() as $customtabsarray)
		{
			$customtabs->fetch($customtabsarray['rowid']);
			$var=!$var;
			print "<tr ".$bc[$var].">";
			print '<td><a href="'.$_SERVER["PHP_SELF"].'?rowid='.$customtabsarray['rowid'].'">';
