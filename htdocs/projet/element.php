...
elseif($action =='multi'){
	...
	if( isset($_POST['delete']) && isset($_POST['type']) )  {
		...
		foreach($_POST['check'][$_POST['type']]  as $row){
			$element = new $classname($db);
			$element->id = $row; 
			$r = $element->delete($row) ; 
...
// var_dump($tablename); 
		...
		print '<form action="'.$_SERVER["PHP_SELF"].'?id='.$projectid.'" method="post">';
		print '<input type="hidden" name="action" value="multi">';
		print '<input type="hidden" name="type" value="'.$classname.'">';
		...
		if (count($elementarray)>0 && is_array($elementarray))
		{
			...
			for ($i = 0; $i < $num; $i++)
			{
				...
				$var=!$var;
				print "<tr ".$bc[$var].">";
				print '<td  align="right" width="20"><input type="checkbox" name="check['.$classname.'][]" value="'.$elementarray[$i].'"  '.(($element->statut<2)?'': 'disabled="disabled"').'></td>';
				// Ref
				print '<td align="left">';
				...
				if ($qualifiedfortotal && ($key != 'invoice' || ($key == 'invoice' && $element->type == 0) ) )
				{
					$tot++;
			...
			print '<tr class="liste_total"><td colspan="4">'.$langs->trans("Number").': '.$tot.'</td>';
			...
			print '<tr><td colspan="7"><input type="submit" name="delete" value="Supprimer"></td></tr>'; 
		...
		if ($project->statut > 0)
		{
			if ($project->societe->prospect || $project->societe->client)
			{
				...
				{
				/// OSCIM MOD 
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/comm2o/propal.php?socid='.$project->societe->id.'&amp;action=create&amp;origin='.$project->element.'&amp;originid='.$project->id.'">'.$langs->trans("AddProp").'</a>';
				}
