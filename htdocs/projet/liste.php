...
{
	$TContactId = GETPOST('sendByMailToServicePaiement');
	if (!empty($TContactId)) sendMail($db, $TContactId);
}

if (GETPOST('publipostage'))
{
	$TProjectId = GETPOST('builddocPublipostage');
	if (!empty($TProjectId)) generateDoc($db, $TProjectId);
}
...
if ($resql)
{
	...
	while ($i < $num)
	{
		...
		if ($userAccess >= 0)
		{
			...
			{
				$fk_socpeople = getIdContactServicePaiement($db, $objp->projectid);
				if ($fk_socpeople) print '<td align="right"><input name="sendByMailToServicePaiement['.$fk_socpeople.'][]" type="checkbox" class="checkbox_sendmail" value="'.$objp->projectid.'" /></td>';
				else print '<td align="right">'.img_picto($langs->transnoentitiesnoconv('Contact [Service Paiement] non trouvé'), 'warning').'</td>';
			}
