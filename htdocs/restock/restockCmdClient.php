...
elseif ($action=="restock")
{
	...
	foreach($tblproduct as $idproduct)
	{
		...
		if ($nbprod > 0)
		{
			...
			{	
				// on met à jours le prix d'acaht en fontion du coef et du prix de vente
				$restock_static=new Restock($db);
...
elseif ($action=="createrestock")
{
	...
	foreach($tblCmdeFourn as $CmdeFourn)
	{
		...
		// si on magouille on modifie le prix fournisseur
		if ($coefcmdclient2fourn > 0)
		{
			$productfournprice = new ProductFournisseur($db);
