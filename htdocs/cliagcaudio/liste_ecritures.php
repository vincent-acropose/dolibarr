...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head,'cliagc_list_ecritures',$langs->trans("FinancialAccount"),0,'account');
	...
	if ($limitsql > $viewline) $navig.='<a href="liste_ecritures.php?'.$param.'&amp;page='.($page+1).'">'.img_previous().'</a>';
	...
	$sql = "SELECT b.rowid, b.dateo as do, b.datev as dv,pay.rowid as id_reglement,payfourn.rowid as id_reglement_fourn,";
	...
	*/
	$sql.= " FROM ".MAIN_DB_PREFIX."bank_account as ba";
	$sql.= ", ".MAIN_DB_PREFIX."bank as b";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."paiement as pay ON (b.rowid = pay.fk_bank)";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."paiementfourn as payfourn ON (b.rowid = payfourn.fk_bank)";
	if ($mode_search)
	{
	...
	if ($result)
	{
		...
		while ($i < $num)
		{
			...
			if ($i >= ($viewline * (($totalPages-$page)-1)))
			{
				...
				// On récupère la liste des factures concernées par le règlement
				print '<td>';
				list($TFactClient, $TThirdClient) = getListFacture($objp->id_reglement);
				print $TFactClient;
				list($TFactFourn, $TThirdFourn) = getListFacture($objp->id_reglement_fourn, 'fourn');
				print $TFactFourn;
				print '</td>';
				...
				if ($objp->rappro && $object->canBeConciliated() > 0)  // If line not conciliated and account can be conciliated
				{
					...
					print '</a>';
					print "&nbsp; ";
					print '<a href="'.dol_buildpath('/bankimport/releve.php?num='.$objp->num_releve.'&amp;account='.$object->id, 1).'">'.$objp->num_releve.'</a>';
					print "</td>";
				}
				else
				{
...
function getListFacture($id_reglement, $fourn='') {
	...
	while($res = $db->fetch_object($resql)) {
		...
		if($f->fetch($res->{'fk_facture'.$fourn}) > 0) {
			...
			$s = new Societe($db);
			$s->fetch($f->socid);
			$TThird[] = $s->getNomUrl(1);
		}
	}
	
	return array(implode('<br />', $Tfact), implode('<br />', $TThird));
