...
$sql.= ', f.fk_projet, p.ref AS projectRef, p.title AS projectName, p.public AS projectPublic, cge.numetape, cge.descriptionetape';
...
$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'projet_customfields pcf ON (pcf.fk_projet = f.fk_projet)';
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans('Projet'));
    ...
    if ($num > 0)
    {
        ...
        while ($i < min($num,$limit))
        {
			...
			$projet->id = $objp->fk_projet;
			$projet->ref = $objp->projectRef;
			$projet->title = $objp->projectName;
			$projet->public = $objp->projectPublic;
			print '<td class="nowrap">';
			if ($projet->id > 0) print $projet->getNomUrl(1, '', 1);
			print '</td>';
			
			// Etape Projet
			print '<td class="nowrap">';
			print wordwrap($objp->numetape.' '.$objp->descriptionetape, 50, '<br />');
			print '</td>';
