...
$search_country=GETPOST('country_id','int');
...
$sql.= ' u.login';
$sql.= ' FROM '.MAIN_DB_PREFIX.'societe as s LEFT JOIN '.MAIN_DB_PREFIX.'c_country pays ON (s.fk_pays = pays.rowid), '.MAIN_DB_PREFIX.'propal as p';
...
if ($search_country > 0)
{
    $sql.= " AND pays.rowid = ".$search_country;
}
...
if ($result)
{
	...
	if ($search_country)		 $param.='&country_id='.$search_country;
	...
	if ($user->rights->societe->client->voir || $socid)
	{
	    $moreforfilter.=$langs->trans('Pays Société'). ': ';
	    $moreforfilter.=$form->select_country();
	}
