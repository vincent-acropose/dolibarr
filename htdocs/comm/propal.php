...
if ($action == 'create')
{
	...
	if ($action == 'clone') {
		...
		$formquestion = array(
							...
							// 1),
							array('type' => 'other','name' => 'socid','label' => $langs->trans("SelectThirdParty"),'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', '(s.client=1 OR s.client=2 OR s.client=3)', '', 0, 0, array(), 0, 'minwidth300')));
