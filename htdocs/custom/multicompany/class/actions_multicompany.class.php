...
class ActionsMulticompany
{
	...
	function doActions(&$action='')
	{
		...
		if ($action == 'add' && empty($cancel) && $user->admin && ! $user->entity)
		{
			...
			if (! $error)
        	{
				...
				$this->dao->options['sharings']['project']	= (GETPOST('project') ? GETPOST('project') : null);
	...
	function formObjectOptions($parameters=false, &$object, &$action='')
	{
		...
		if (($user->admin && ! $user->entity) && is_object($object) && $object->element == 'societe')
		{
	 ...
	 *	@return	string
	 */
	function select_entities($selected='', $htmlname='entity', $option='', $login=0, $exclude=false, $emptyvalue=false)
	{
		global $user,$langs;
	...
	function assign_values($action='view')
	{
		...
		if ($action == 'create' || $action == 'edit')
		{
			...
			$this->tpl['multiselect_shared_project'] = $this->multiselect_entities('project', $this->dao);
		}
		else
		{
