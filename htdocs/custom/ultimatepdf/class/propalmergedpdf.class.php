...
class Propalmergedpdf extends CommonObject
{
	...
	var $element='propal_merged_pdf';			//!< Id that identify managed objects
	var $table_element='propal_merged_pdf';		//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."propal_merged_pdf(";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."propal_merged_pdf");
    ...
    function fetch($id)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."propal_merged_pdf as t";
     ...
     *  @return int          	<0 if KO, >0 if OK
     */
    function fetch_by_propal($propal_id, $lang='')
    {
    	global $langs,$conf;
    	...
    	dol_syslog(get_class($this)."::fetch_by_propal sql=".$sql, LOG_DEBUG);
    	$resql=$this->db->query($sql);
    	if ($resql)
    	{
    		if ($this->db->num_rows($resql))
    	...
    	{
    		$this->error="Error ".$this->db->lasterror();
    		dol_syslog(get_class($this)."::fetch_by_propal ".$this->error, LOG_ERR);
    		return -1;
    	}
    }
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."propal_merged_pdf SET";
	...
	function delete($user, $notrigger=0)
	{
		...
		if (! $error)
		{
    		$sql = "DELETE FROM ".MAIN_DB_PREFIX."propal_merged_pdf";
    		$sql.= " WHERE rowid=".$this->id;
	 ...
	 *  @return	int					 <0 if KO, >0 if OK
	 */
	function delete_by_propal($user, $propal_id, $lang_id='',  $notrigger=0)
	{
		global $conf, $langs;
		$error=0;
