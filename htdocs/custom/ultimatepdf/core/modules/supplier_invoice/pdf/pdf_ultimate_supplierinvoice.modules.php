...
class pdf_ultimate_supplierinvoice extends ModelePDFSuppliersInvoices
{
	...
	function __construct($db,$object)
	{
		...
		$this->postotalht=174;
		if (! ($conf->global->ULTIMATE_GENERATE_DOCUMENTS_WITHOUT_VAT == "no")) $this->posxtva=$this->posxup;
		...
		if ($conf->fournisseur->dir_output.'/facture')
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					$pdf->MultiCell($this->posxqty-$this->posxup, 3, price($object->lines[$i]->pu_ht), 0, 'C', 0);

					// Quantity
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxdiscount-$this->posxqty-0.8, 3, $object->lines[$i]->qty, 0, 'C');
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$deltax=$this->marge_gauche+$this->page_largeur/2;
		$pdf->RoundedRect($deltax, $tab2_top, $this->page_largeur/2 - $this->marge_droite - $this->marge_gauche, 20, 2, $round_corner = '1111', 'F', $this->style, $bgcolor);
	...
	{
		return pdf_ultimatepagefoot($pdf,$outputlangs,'SUPPLIER_INVOICE_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,0,$hidefreetext);
	}
