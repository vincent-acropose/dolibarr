...
/**
 *	\class      pdf_ultimate_weight_order1
 *	\brief      Class for generate the commands with ultimate_weight_order1 model
 */
class pdf_ultimate_weight_order1 extends ModelePDFCommandes
{
	var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->posxweight=$this->posxtva-1 - (empty($conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH)?20:$conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH);	// width of images
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->commande->dir_output)
		{
			...
			{	
				...
				if (! empty($conf->global->MAIN_DISPLAY_ORDER_AGREEMENT_BLOCK))
				{
				 ...
				 for ($i = 0 ; $i < $nblignes ; $i++)
                {
					...
					if ($conf->global->ULTIMATE_DOCUMENTS_WITH_REF != "yes")
					{
						pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxweight-$curX,3,$curX,$curY,$hideref,$hidedesc);
					}
					else
					{
						...
						pdf_writelinedesc_ref($pdf,$object,$i,$outputlangs,$this->posxweight-$curX,3,$curX,$curY,$hideref,$hidedesc,0,'label');
					}
					...
					$pdf->MultiCell($this->posxtva-$this->posxweight-1, 3, $weight, 0, 'C');
					...
					if (empty($conf->global->ULTIMATE_SHOW_HIDE_QTY))
					{
						...
						$pdf->MultiCell($this->posxdiscount-$this->posxqty-0.8, 3, $qty, 0, 'C');	// Enough for 6 chars
					}
				...
				if (! empty ( $object->id ) && !(in_array($object->id, $already_merged))) {
					...
					if ($conf->global->MAIN_MULTILANGS) {
						$filetomerge->fetch_by_order ( $object->id, $outputlangs->defaultlang);
					} else {
						$filetomerge->fetch_by_order ( $object->id );
					}
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
        ...
        if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')
        {
	        if (! empty($object->fk_bank) || ! empty($conf->global->FACTURE_RIB_NUMBER))
	        {
                ...
                $posy=pdf_ultimate_orders_bank($pdf,$outputlangs,$curx,$cury,$account,0,$default_font_size);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxtva-$this->posxweight-1,6, $outputlangs->transnoentities("Weight"),0,'C',1);
		}	
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetTextColorArray($textcolor);
		// Example using extrafields
		$title=(empty($object->array_options['options_nomqcq']))?$outputlangs->transnoentities("Order"):($object->array_options['options_nomqcq']);
		...
		if ($showaddress)
		{
	        ...
	        $pdf->SetTextColorArray($textcolor);
	        $text='<div style="line-height:90%;">'.$outputlangs->transnoentities("PaymentConditions").'</div>';
	        $pdf->writeHTMLCell($width, 5,$this->marge_gauche,$RoundedRectHeight, $text, 0, 0, false, true, 'C', true);
	        ...
	        if ($object->cond_reglement_code || $object->cond_reglement)
	        {
	        	...
	        	$pdf->MultiCell($width, 6, $lib_condition_paiement, '0', 'C');
	        }
			else
			{
				$pdf->SetFont('','', $default_font_size - 2);
	        ...
	        $pdf->SetTextColorArray($textcolor);
	        $text='<div style="line-height:90%;">'.$outputlangs->transnoentities("CommercialInterlocutor").'</div>';
	        $pdf->writeHTMLCell($width, 5,$this->marge_gauche+$width*2+4,$RoundedRectHeight, $text, 0, 0, false, true, 'C', true);
	...
	{
		return pdf_ultimatepagefoot($pdf,$outputlangs,'COMMANDE_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,1,$hidefreetext);
	}
