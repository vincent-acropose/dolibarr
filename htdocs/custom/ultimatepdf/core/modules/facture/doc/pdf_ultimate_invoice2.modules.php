...
class pdf_ultimate_invoice2 extends ModelePDFFactures
{
	var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->posxtva=103;
		$this->posxup=115;		
		$this->posxdiscount=133;
		$this->posxupafter=147;
		$this->posxqty=167;
		$this->postotalht=177;
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					$pdf->MultiCell($this->posxdiscount-$this->posxup-1, 4, $up_excl_tax, 0, 'C', 0);					
					// Discount on line					
					if ($object->lines[$i]->remise_percent)
					{
						$pdf->SetXY ($this->posxdiscount, $curY);
						$remise_percent = pdf_getlineremisepercent($object, $i, $outputlangs, $hidedetails);
						$pdf->MultiCell($this->posxupafter-$this->posxdiscount-1, 4, $remise_percent, 0, 'C');
					}
					...
					{
						$up_after = price(0);
						$pdf->SetXY ($this->posxupafter-1, $curY);
						$pdf->MultiCell($this->posxqty-$this->posxupafter, 4, $up_after, 0, 'C');
					}
					else
					{
						if ($object->lines[$i]->remise_percent > 0)
						{
							...
							$pdf->SetXY ($this->posxupafter-1, $curY);
							$pdf->MultiCell($this->posxqty-$this->posxupafter, 4, price($up_after), 0, 'C');
						}
					}
					...
					if (empty($conf->global->ULTIMATE_SHOW_HIDE_QTY))
					{
						...
						$pdf->MultiCell($this->postotalht-$this->posxqty-1, 4, $qty, 0, 'C');	
					}
					...
					if (empty($conf->global->ULTIMATE_SHOW_HIDE_THT))
					{
						...
						$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht+3, 4, $total_excl_tax, 0, 'R', 0);
					}
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$posxval=$this->page_largeur/2 - $this->marge_gauche;
		...
		if ($object->type != 2)
		{
				...
				|| ($object->mode_reglement_code == 'VIR' && empty($conf->global->FACTURE_RIB_NUMBER)))
			{
				...
				$pdf->MultiCell($posxval, 3, $this->error,0,'L',0);
				$pdf->SetTextColorArray($textcolor);;
			...
			&& $object->mode_reglement_code != 'VIR')
			{
				...
				$pdf->MultiCell($posxval, 5, $titre, 0, 'L');

				$pdf->SetFont('','', $default_font_size - 2);
				$pdf->SetXY($posxval, $posy);
				$lib_mode_reg=$outputlangs->transnoentities("PaymentType".$object->mode_reglement_code)!=('PaymentType'.$object->mode_reglement_code)?$outputlangs->transnoentities("PaymentType".$object->mode_reglement_code):$outputlangs->convToOutputCharset($object->mode_reglement);
				$pdf->MultiCell($posxval, 5, $lib_mode_reg,0,'L');
			...
			if (! empty($conf->global->ULTIMATEPDF_GENERATE_INVOICES_WITH_AUTO_LIQUIDATION))
			{
				...
				$pdf->MultiCell($posxval, 4, $titre1, 0, 'L');
				$pdf->SetFont('','', $default_font_size - 2);
				$pdf->SetXY(70, $posy);
			...
			if (is_array ( $extralabels ) && key_exists ( 'newline', $extralabels ) && !empty($title_key)) {
				...
				$pdf->MultiCell($posxval, 5, $title, 0, 'L');
				$posy=$pdf->GetY()+2;
			}
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'CHQ')
			{
				...
				if (! empty($conf->global->FACTURE_CHQ_NUMBER))
				{
					...
					if ($conf->global->FACTURE_CHQ_NUMBER > 0)
					{
						...
						$pdf->MultiCell($posxval, 3, $outputlangs->transnoentities('PaymentByChequeOrderedTo',$account->proprio),0,'L',0);
						$posy=$pdf->GetY()+1;
			            ...
			            if (empty($conf->global->MAIN_PDF_HIDE_CHQ_ADDRESS))
			            {
							...
							$pdf->MultiCell($posxval, 3, $outputlangs->convToOutputCharset($account->owner_address), 0, 'L', 0);
							$posy=$pdf->GetY()+2;
			            }
					}
					if ($conf->global->FACTURE_CHQ_NUMBER == -1)
					{
						$pdf->SetXY($this->marge_gauche, $posy);
						$pdf->SetFont('','B', $default_font_size - $diffsizetitle);
						$pdf->MultiCell($posxval, 3, $outputlangs->transnoentities('PaymentByChequeOrderedTo',$this->emetteur->name),0,'L',0);
						$posy=$pdf->GetY()+1;
			            ...
			            if (empty($conf->global->MAIN_PDF_HIDE_CHQ_ADDRESS))
			            {
							$pdf->SetXY($this->marge_gauche, $posy);
							$pdf->SetFont('','', $default_font_size - $diffsizetitle);
							$pdf->MultiCell($posxval, 3, $outputlangs->convToOutputCharset($this->emetteur->getFullAddress()), 0, 'L', 0);
							$posy=$pdf->GetY()+2;
			            }
					}
				}
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($object->total_ht + (! empty($object->remise)?$object->remise:0)), 0, $userlang,0,-1,-1,$currency_code), 0, 'R', 1);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		{		
			...
			{			
				...
				$pdf->MultiCell($this->posxdesc,6, $outputlangs->transnoentities("Ref"),0,'C',1);
				$pdf->SetXY ($this->posxdesc-1, $tab_top);
				$pdf->MultiCell(110,6, $outputlangs->transnoentities("Designation"), 0, 'L', 1);
			}
			else			
			{			
				$pdf->SetXY ($this->posxdesc-1, $tab_top);
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(14,6, $outputlangs->transnoentities("Discount"), 0, 'C', 1);
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->postotalht-$this->posxqty,6, $outputlangs->transnoentities("Qty"), 0, 'C', 1);
		}

        $pdf->line($this->postotalht-1, $tab_top, $this->postotalht-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY ($this->postotalht-6, $tab_top);
			$pdf->MultiCell(29,6, $outputlangs->transnoentities("AmountHT"), 0, 'C', 1);
		}
