...
$pageprev = $page - 1;
$pagenext = $page + 1;
if (! $sortfield) $sortfield='c.datep';
...
if ($resql)
{
	...
	if ($socid)
	{
		$soc = new Societe($db);
		$soc->fetch($socid);
		$title = $langs->trans('ListOfPropals') . ' - '.$soc->nom;
	}
	else
	{
	...
	while ($i < $num)
	{
		...
		if (($orders_list[$i]->fk_statut = 2) && $db->jdate($orders_list[$i]->date_valid) < ($now - $conf->propal->facturation->warning_delay)) print img_picto($langs->trans("Late"),"warning");
		print '</td>';
