...
if ($action == 'confirm_massinvoicing' && $confirm == 'yes')
{
	...
	foreach ($array as $comp_id => $value){
		...
		if(($conf->global->MASSO_AUTO_PDF || $conf->global->MASSO_AUTO_MAIL) && $facid){
			...
			$result = supplier_invoice_pdf_create($db, $fac, $fac->modelpdf, $outputlangs);
			if(! $result) $error++;
		}
...
if ($search_sale > 0 || (! $user->rights->fournisseur->lire && ! $socid)) $sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
if ($search_user > 0)
{
...
if (!$user->rights->fournisseur->lire && !$socid) $sql.= " AND s.rowid = sc.fk_soc AND sc.fk_user = " .$user->id;
if ($sref)
{
...
if ($resql)
{
	...
	if ($sref_supplier)     $param.='&sref_supplier='.$sref_supplier;
	...
	if($user->rights->fournisseur->facture->valider && $conf->global->STOCK_CALCULATE_ON_SUPPLIER_BILL == 0){
	...
	print_liste_field_titre($langs->trans('RefSupplierOrder'),$_SERVER["PHP_SELF"],'c.ref_supplier','',$param,'',$sortfield,$sortorder);
	...
	while ($i < $num)
	{
		...
		if (($orders_list[$i]->fk_statut > 0) && ($orders_list[$i]->fk_statut < 5) && $db->jdate($orders_list[$i]->date_valid) < ($now - $conf->commande->fournisseur->warning_delay)) print img_picto($langs->trans("Late"),"warning");
		print '</td>';
